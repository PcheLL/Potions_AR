...
public class SaveInstantTargetActivity extends Activity implements InternalRendering, InstantTrackerListener {

    private CustomRenderExtension renderExtension;
    private View heightSettingsLayout;
    private Button changeStateButton;

    private WikitudeSDK wikitudeSDK;
    private InstantTrackingState currentTrackingState;
    ...
    protected void onCreate(@Nullable final Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_instant_save_target);
        ...
        findViewById(R.id.save_button).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(final View v) {
                instantTracker.saveCurrentInstantTarget(
                        // The saved file can be usually found in /sdcard/Android/data/com.wikitude.nativesdksampleapp/ (getExternalFilesDir)
                        new File(getExternalFilesDir(null), "savedTarget.wto").getAbsolutePath(),
                        new CompletionCallback() {
                            @Override
                            public void onCompletion() {
                                Toast.makeText(SaveInstantTargetActivity.this, "Saving was successful.", Toast.LENGTH_SHORT).show();
                            }
                        },
                        new ErrorCallback() {
                            @Override
                            public void onError(@NonNull final WikitudeError error) {
                                Toast.makeText(SaveInstantTargetActivity.this, "Saving failed: " + error.getMessage(), Toast.LENGTH_LONG).show();
                            }
                        },
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (state == InstantTrackingState.Tracking) {
                    ...
                    renderExtension.getStrokedRectangle().setColor(1.0f, 0.58f, 0.16f);
                } else {
    ...
    public void onStateChangeFailed(final InstantTracker tracker, final InstantTrackingState failedState, final WikitudeError error) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                new AlertDialog.Builder(SaveInstantTargetActivity.this)
    ...
    public void onInitializationPoseChanged(final InstantTracker tracker, final InitializationPose pose) {
        renderExtension.setCurrentlyRecognizedTarget(pose);

        if (instantTracker.canStartTracking()) {
            renderExtension.getStrokedRectangle().setColor(0.0f, 1.0f, 0.0f);
        } else {
            renderExtension.getStrokedRectangle().setColor(1.0f, 0.0f, 0.0f);
        }
    }

    @Override
    public void onTrackingStarted(final InstantTracker tracker, final InstantTarget target) {
